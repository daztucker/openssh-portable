...
platform_locked_account(struct passwd *pw)
{
	...
	if (passwd && *passwd) {
		...
		if (strcmp(passwd, LOCKED_PASSWD_STRING) == 0) {
			debug3_f("password '%s' matches locked string '%s'",
			    passwd, LOCKED_PASSWD_STRING);
			locked = 1;
		}
#endif
		...
		if (strncmp(passwd, LOCKED_PASSWD_PREFIX,
			...
			debug3_f("password '%s' matches locked prefix '%s'",
			    passwd, LOCKED_PASSWD_PREFIX);
			locked = 1;
		}
#endif
